...
for rep in range(N):
    ...
    while t<T:
        ...
        if len(active_arm)>1:
            ...
            conf_bnd = np.sqrt(sigma**2*eta_p*np.log(T))+np.sqrt(sigma_c**2*np.log(T)/(4*M)) #the constants are tuned from the original ones in the paper to get better performance
